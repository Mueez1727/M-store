...
export default function SalesManagementSystem() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Load purchases
        const savedPurchases = await window.storage.get("m-store-purchases");
        if (savedPurchases && savedPurchases.value) {
          setPurchases(JSON.parse(savedPurchases.value));
        }

        // Load sales
        const savedSales = await window.storage.get("m-store-sales");
        if (savedSales && savedSales.value) {
          setSales(JSON.parse(savedSales.value));
        }
      } catch (e) {
  ...
  useEffect(() => {
    const saveData = async () => {
      try {
        ...
        await window.storage.set("m-store-sales", JSON.stringify(sales));
      } catch (e) {
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-3">
          ...
          <input
            ...
            value={
              dataType === "purchase" ? formData.purchasedFrom : formData.soldTo
            }
            onChange={(e) =>
              dataType === "purchase"
  ...
  const DateAccordion = React.memo(
    ({ dateKey, label, dataType }) => {
      const isExpanded = expandedDays[dateKey];
      const data =
        dataType === "purchase" ? purchases[dateKey] : sales[dateKey];
      ...
      if (!isExpanded) {
        return (
          <div
            className={`border ${borderColor} rounded-lg mb-3 overflow-hidden`}
          >
            <button
            ...
            >
              <span className="font-semibold">
                {label}{" "}
                {isToday && (
                  <span className="text-xs ml-2 bg-green-500 text-white px-2 py-1 rounded">
                    Today
                  </span>
                )}
              </span>
                    ...
                    <tbody>
                      {data.map((item, idx) => (
                        <tr key={idx} className={`border-b ${borderColor}`}>
                          ...
                          {dataType === "sale" && (
                            <td className="px-3 py-2 text-right">
                              Rs {parseFloat(item.recovery || 0).toFixed(2)}
                            </td>
                          )}
                          {isToday && (
                            <td className="px-3 py-2 text-center">
                              <button
                                onClick={() =>
                                  dataType === "purchase"
                                    ? deletePurchase(dateKey, idx)
                                    : deleteSale(dateKey, idx)
                                }
    ...
    (prevProps, nextProps) => {
      ...
      return (
        ...
        JSON.stringify(prevProps.expandedDays) ===
          JSON.stringify(nextProps.expandedDays) &&
  ...
  return (
    <div className={`min-h-screen ${bgColor} ${textColor}`}>
      ...
      <nav
        className={`${cardBg} border-b ${borderColor} sticky top-0 z-50 shadow-md`}
      >
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {currentPage === "dashboard" && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div
                onClick={() => {
                  setCurrentPage("purchasing");
                  setSelectedMonth(null);
                  setExpandedDays({});
                }}
                className={`${cardBg} p-8 rounded-lg shadow-lg cursor-pointer hover:shadow-xl transition transform hover:scale-105 border ${borderColor}`}
              >
            ...
            {/* Stats Table */}
            <div
              className={`${cardBg} rounded-lg shadow-lg p-6 border ${borderColor} overflow-x-auto`}
            >
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {["daily", "weekly", "monthly", "overall"].map((period) => {
                    ...
                    const totalRecovery = filteredSales.reduce(
                      (sum, item) => sum + (parseFloat(item.recovery) || 0),
                      0
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-right">
                          Rs {purchaseStats.amount.toFixed(2)}
                        </td>
                        <td className="px-4 py-2 text-right">
                          {purchaseStats.items}
                        </td>
                        <td className="px-4 py-2 text-right">
                          Rs {saleStats.amount.toFixed(2)}
                        </td>
                        <td className="px-4 py-2 text-right">
                          {saleStats.items}
                        </td>
                        <td className="px-4 py-2 text-right font-bold text-green-500">
                          Rs {totalRecovery.toFixed(2)}
                        </td>
                      </tr>
                    );
                  })}
              ...
              <button
                onClick={() => downloadExcel("stats", "overall")}
        ...
        {currentPage === "purchasing" && (
          <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {months.map((month, idx) => (
                  <div
                    ...
                    className={`${cardBg} p-4 rounded-lg shadow-md cursor-pointer hover:shadow-lg transition border ${borderColor}`}
                  >
            ...
            {selectedMonth !== null && (
              <div
                className={`${cardBg} rounded-lg shadow-lg p-6 border ${borderColor}`}
              >
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-2xl font-bold">
                    {months[selectedMonth]} - Purchasing Details
                  </h3>
                  <button
                ...
                <button
                  onClick={() => downloadExcel("purchase", "monthly")}
          ...
          <>
            ...
            {selectedMonth !== null && (
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-2xl font-bold">
                    {months[selectedMonth]} - Selling Details
                  </h3>
                  <button
                    onClick={() => setSelectedMonth(null)}
                    className="flex items-center gap-2 text-blue-500 hover:text-blue-600"
                ...
                <button
                  onClick={() => downloadExcel("sale", "monthly")}
                  className="mt-6 flex items-center gap-2 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition"
                >
                  <Download size={20} /> Download Excel
                </button>
        ...
        {currentPage === "details" && (
          <>
            ...
            {/* Purchasing Details */}
            <div
              className={`${cardBg} rounded-lg shadow-lg p-6 border ${borderColor} mb-8`}
            >
              <h3 className="text-2xl font-bold mb-6">
                Purchasing Details by Supplier
              </h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {Object.entries(aggregatePurchasingDetails()).map(
                      ([supplier, data]) => (
                        <tr
                          key={supplier}
                          className={`border-b ${borderColor} hover:${
            ...
            >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {Object.entries(aggregateSellingDetails()).map(
                      ([buyer, data]) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right text-green-600">
                              Rs {data.totalRecovery.toFixed(2)}
                            </td>
